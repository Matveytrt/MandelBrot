...
// g++ -O3 -march=haswell -mtune=intel -mavx2 -funroll-loops -o ymm mndlbrt_ymm.cpp -lraylib
...
void DrawFractal(int wdth, int hght, int max_iter)
{
    ...
    __m256 l2max = _mm256_set1_ps(lmax * lmax);
    float xval[_STEP_] = {};
    ...
    #endif
    {
        ...
        {
            __m256 y = _mm256_set1_ps(y_min + py * ay);

            for (int px = 0; px < wdth; px += _STEP_) 
            {
                for (int idx = 0; idx < _STEP_; idx++) { xval[idx] = x_min + (px + idx) * ax;}
                __m256 x = _mm256_set_ps(xval[7], xval[6], xval[5], xval[4], xval[3], xval[2], xval[1], xval[0]);

                __m256 zx = x;
                __m256 zy = y;
                __m256 cmp = _mm256_set1_ps(0xffffffff);
                __m256i iter_data = _mm256_set1_epi32(0);
                ...
                for (int iter = 0; iter < max_iter; iter++)
                {
                    ...
                    zx = _mm256_add_ps(_mm256_sub_ps(x2, y2), x);
                    zy = _mm256_add_ps(_mm256_add_ps(xy, xy), y);

                    iter_data = _mm256_sub_epi32(iter_data, _mm256_castps_si256(cmp));
                    
                    __m256 r2 = _mm256_add_ps(x2, y2);
                    cmp = _mm256_cmp_ps(r2, l2max, _CMP_LT_OS);

                    int mask = _mm256_movemask_ps(cmp) & 0xF;

                    if (mask == 0) { break; }
                }

                int iter_vals[_STEP_] = {};
                _mm256_storeu_si256((__m256i*)iter_vals, iter_data);
